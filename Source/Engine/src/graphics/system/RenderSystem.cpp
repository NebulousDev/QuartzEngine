...
namespace Quartz
{
	...
	void RenderSystem::CreatePipeline()
	{
		...
		DescriptorAttachment diffuseTextureAttachment;
		diffuseTextureAttachment.binding = 1;
		diffuseTextureAttachment.arraySize = 1;
		diffuseTextureAttachment.type = DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		diffuseTextureAttachment.stages = SHADER_STAGE_FRAGMENT;

		pipelineState.descriptorAttachents.PushBack(uniformAttachment);
		pipelineState.descriptorAttachents.PushBack(diffuseTextureAttachment);
	...
	void RenderSystem::OnInit(EntityWorld& world)
	{
		...
		for (UInt32 i = 0; i < mpSurface->GetBackBufferCount(); i++)
		{
			...
			//mpGfx->BindImageView(*mpGraphicsPipeline, *pGraphicsCommandBuffer, mpDiffuse);
